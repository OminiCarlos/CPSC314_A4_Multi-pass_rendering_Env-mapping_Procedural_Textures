...
// shadowCam.position.set(0.0, 15.0, 0.0);
shadowCam.lookAt(scene.position);
shadowScene.add(shadowCam);

const shadowCamHelperGeometry = new THREE.SphereGeometry(1, 10, 10);
const shadowCamHelperMaterial = new THREE.MeshBasicMaterial({
  color: 0xffffff
});
const shadowCamHelper = new THREE.Mesh(
  shadowCamHelperGeometry,
  shadowCamHelperMaterial
);
console.log(shadowCam.position);
...
function update() {
  ...
  renderer.clear();
  shadowCamHelper.position.copy(shadowCam.position);
